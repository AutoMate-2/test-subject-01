@tointegration
Feature: Catalogue/GetDepartureCities - CL-T5

@smoke @api @catalogue @positive
Scenario: CL-T5-012 - Get departure cities for Kazakhstan
...
Scenario: CL-T5-013 - Get departure cities without country code
  Given I am authenticated with valid token
  When I send a GET request to "/api/Catalogue/GetDepartureCities"
  Then the response status code should be 400

@regression @api @catalogue @positive
Scenario: CL-T5-014 - Verify Kazakhstan departure cities include major cities
  Given I am authenticated with valid token
  When I send a GET request to "/api/Catalogue/GetDepartureCities?countryCode=KZ"
  Then the response status code should be 200
...
Scenario: CL-T5-015 - Get departure cities with invalid country code
  Given I am authenticated with valid token
...
Scenario Outline: CL-T5-016 - Get departure cities with boundary country codes
  Given I am authenticated with valid token
...
@regression @api @catalogue @positive
Scenario: CL-T5-017 - Verify cityUID is valid UUID format
  Given I am authenticated with valid token
  When I send a GET request to "/api/Catalogue/GetDepartureCities?countryCode=KZ"
  Then the response status code should be 200
  And each cityUID should be a valid UUID format for country "KZ"

@regression @api @catalogue @positive
Scenario: CL-T5-018 - Verify timeZoneOffset values
  Given I am authenticated with valid token
  When I send a GET request to "/api/Catalogue/GetDepartureCities?countryCode=KZ"
  Then the response status code should be 200
  And each timeZoneOffset should be a number between -12 and 14 for country "KZ"
